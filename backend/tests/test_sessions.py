...
Tests:
  ...
  4. Sessions list     → returns newly created session
  5. PCM → float32     → silence maps to all-zero float32
  6. PCM → float32     → Int16 max maps to ~1.0 float32
  7. count_words       → correct word counts
  8. build_preview     → short text unchanged, long text truncated
"""
...
async def test_list_sessions_empty(client: AsyncClient):
    """GET /sessions/ returns an empty list when the DB has no rows."""
...
async def test_create_and_retrieve_session(client: AsyncClient, db_session: AsyncSession):
    ...
    expected_word_count = count_words(transcript)   # derive from the real function
    ...
    assert data["transcript_preview"] == transcript
...
async def test_list_sessions_returns_created(client: AsyncClient, db_session: AsyncSession):
    """GET /sessions/ includes the session we just created."""
    session = await SessionService.create_session(db_session)
    await SessionService.finalise_session(
        db=db_session,
        session_id=session.id,
...
def test_pcm_bytes_to_float32_silence():
    """All-zero PCM bytes produce an all-zero float32 array."""
...
def test_pcm_bytes_to_float32_max_positive():
    """Int16 max (32767) maps to ~1.0 float32."""
